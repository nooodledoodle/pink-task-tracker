...
type Task = { id: string; title: string; project: string; status: Status; priority: Priority; due: string; notes: string; duration: number };
...
const nav = [
  ["Dashboard","⌂"],["Projects","♡"],["Table View","☷"],["Calendar","□"],["Kanban","▦"],["Eisenhower","◇"],["Daily Planner","☀"],["Weekly Planner","▤"]
];
...
function App(){
  ...
  useEffect(()=>{
    if(!window.firebase){setSyncState("Offline — saved on this device");return}
  ...
  const save=(t:Task)=>{setTasks(x=>x.some(a=>a.id===t.id)?x.map(a=>a.id===t.id?t:a):[t,...x]);setEditing(null)};
  ...
  return <div className="app-shell">
    ...
    <main>
      <header><button className="menu-btn" onClick={()=>setMenu(!menu)} aria-label="Open menu">☰</button><div><p className="eyebrow">MY WORKSPACE</p><h1>{view}</h1></div><div className="header-actions"><label className="search">⌕<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search tasks..."/></label><button className={user?"account-button signed-in":"account-button"} onClick={()=>user?window.firebase.auth().signOut():setAuthOpen(true)} title={user?"Click to sign out":"Sign in to sync"}>{user?user.email?.slice(0,1).toUpperCase():"Sign in"}</button><button className="primary" onClick={add}>＋ New task</button></div></header>
      ...
      {view==="Table View"&&<Table tasks={shown} onEdit={setEditing}/>} 
...
function Table({tasks,onEdit}:{tasks:Task[],onEdit:(t:Task)=>void}){return <div className="content"><section className="panel table-panel"><div className="panel-title"><div><p className="eyebrow">MASTER LIST</p><h3>All action items</h3></div><span>{tasks.length} results</span></div><div className="table-scroll"><table><thead><tr><th>Task</th><th>Project</th><th>Status</th><th>Priority</th><th>Due date</th></tr></thead><tbody>{tasks.map(t=><tr key={t.id} onClick={()=>onEdit(t)}><td><strong>{t.title}</strong></td><td><span className={`project-pill ${projectClass(t.project)}`}>{t.project}</span></td><td><Badge value={t.status} type="status"/></td><td><Badge value={t.priority} type="priority"/></td><td>{new Date(t.due+"T12:00").toLocaleDateString("en-US",{month:"short",day:"numeric"})}</td></tr>)}</tbody></table></div></section></div>}
